...
- **Single MCP server running at a time (mandatory).** Mirroring dtctl's single-context model, only one MCP server may be running at any moment. On every `"switch to <NICKNAME>"`:
  1. Stop every currently-running MCP server with `run_vscode_command` → `workbench.mcp.stopServer` (args: `["<server-name>"]`). Verified working from the agent.
  2. If the target nickname has a matching MCP server entry in `.vscode/mcp.json` / `.mcp.json`, start it with `run_vscode_command` → `workbench.mcp.startServer` (args: `["<server-name>"]`). Note: tool visibility may lag a turn after start — the user can verify via *MCP: List Servers*.
  3. If the target nickname has no MCP server entry (dtctl-only tenant), leave **no** MCP server running. The agent operates dtctl-only for that tenant.
  4. If the target nickname has only an MCP server entry (no dtctl context), start that one server and operate MCP-only.
